    ...
    def __post_init__(self):
        if self.dataset_name is None and self.train_file is None and self.validation_file is None:
            ...
            if self.train_file is not None:
                ...
                if extension not in ["csv", "jsonl", "txt"]:
                    raise ValueError("`train_file` should be a csv, a jsonl or a txt file.")
            if self.validation_file is not None:
                extension = self.validation_file.split(".")[-1]
                if extension not in ["csv", "jsonl", "txt"]:
                    raise ValueError("`validation_file` should be a csv, a jsonl or a txt file.")

def parse_jsonl_data(jsonl_path):
    data = []
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".jsonl"):
        # If we pass only one argument to the script and it's the path to a jsonl file,
        # let's parse it to get our arguments.
        #model_args, data_args, training_args, adapter_args = parser.parse_json_file(
        #    json_file=os.path.abspath(sys.argv[1])
        #)
        # Parse JSON Lines configuration file
            ...
            for line_number, line in enumerate(f, start=1):
                ...
                if line:
                    try:
                        ...
                        combined_args = merge_dicts(combined_args, args)
                    except json.JSONDecodeError as e:
                        print(f"Error parsing JSON on line {line_number}: {e}")
                        sys.exit(1)
        # Parse the combined arguments
        try:
            model_args, data_args, training_args, adapter_args = parser.parse_dict(combined_args)
